...
class PortfolioManager {
  ...
  renderHoldings() {
    ...
    tbody.innerHTML = this.positions.map(position => `
      <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
        ...
        <td class="py-4 text-right font-semibold">${utils.formatCurrency(position.marketValue)}</td>
        <td class="py-4 text-right ${position.gainLoss >= 0 ? 'text-green-600' : 'text-red-600'}">
          ${position.gainLoss >= 0 ? '+' : ''}${utils.formatCurrency(position.gainLoss)}
        </td>
        <td class="py-4 text-right ${position.gainLossPercent >= 0 ? 'text-green-600' : 'text-red-600'}">
          ${position.gainLossPercent >= 0 ? '+' : ''}${position.gainLossPercent.toFixed(2)}%
        </td>
        <td class="py-4 text-center">
          <button class="text-blue-600 hover:text-blue-800 mr-2" onclick="portfolioManager.viewDetails('${position.symbol}')">
            <i class="fas fa-eye"></i>
          </button>
          <button class="text-green-600 hover:text-green-800 mr-2" onclick="portfolioManager.buyMore('${position.symbol}')">
            <i class="fas fa-plus"></i>
          </button>
          <button class="text-red-600 hover:text-red-800" onclick="portfolioManager.sell('${position.symbol}')">
  ...
  updatePortfolioMetrics() {
    ...
    const elements = {
      ...
      totalGainPercent: document.querySelector('[data-metric="total-gain-percent"]')
    };
  ...
  renderTransactions(transactions) {
    ...
    tbody.innerHTML = transactions.map(tx => `
      <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
        ...
        <td class="py-3">
          <span class="px-2 py-1 ${tx.type === 'BUY' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'} rounded text-xs font-semibold">
            ${tx.type}
          </span>
        </td>
        ...
        <td class="py-3 text-right font-semibold">${utils.formatCurrency(tx.total)}</td>
        <td class="py-3">
          <span class="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs">${tx.status}</span>
        </td>
      </tr>
    `).join('');
  }
